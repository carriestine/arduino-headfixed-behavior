...
// session parameters *************************************************************************************************************
 // general parameters -------
  ...
  static unsigned long baseline_time = 300000;            // delay time until first trial
  
  // iti range (uniform random)
  static unsigned long min_iti = 45000;           // minimum iti (ms) 
  static unsigned long max_iti = 75000;           // maximum iti (ms)
...
void loop() {
 ...
 // session initialization (runs once at start) -----------------------------------------------------------------
  if(first_loop){
    ...
    ts_trial_start = ts + baseline_time;           // calculate first trial start
 ...
 if(ts >= ts_trial_end && ts_trial_end != 0){
    ...
    Serial.print(trial_id); Serial.print(" "); Serial.println(ts);
    Serial.print(126);      Serial.print(" "); Serial.println(count_trial);
